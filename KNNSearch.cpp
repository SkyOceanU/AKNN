...
void KNNSearch::create_graph(char* filename,int num){
	vector<Candidate_Pool> pool(k);  //定义候选池
	vector<vector<unsigned > >graph; //定义图
	DistanceL2* distance_ = new DistanceL2();
	graph.resize(base_data.size());
	for(size_t i=0;i<base_data.size();i++){
		pool.clear();
		for(size_t j=0;j<k;j++){
			double dist = (double)distance_->compare(base_data[i].data(),base_data[knn_graph[i][j]].data(),dim);
			Candidate_Pool point = {knn_graph[i][j],dist};
			pool.push_back(point);
		}
		sort(pool.begin(),pool.end(),LessSort_scan);
		graph[i].resize(num);
		for(size_t z=0;z<num;z++){
			graph[i][z] = pool[z].index;
		}
		cout<<i<<endl;
	}
	ofstream out(filename, ios::binary | ios::out);
  	for (unsigned i = 0; i < graph.size(); i++) {
    	...
    	out.write((char*)graph[i].data(), num * sizeof(unsigned));
 	 }
  	out.close();
}
...
//	//主要用于生成指定大小的knn图 
//	search.load_data("file/sift_base.fvecs",search.get_base_data());
//  	search.load_graph("file/sift_100NN_100.graph");
//  	search.create_graph("file/sift_10NN_10.graph",10);
  	return 0;
}
