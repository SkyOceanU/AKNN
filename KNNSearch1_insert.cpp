		...
		while(index < L){
			...
			for(size_t j = 0;j<k;j++){
				if(isExist[knn_graph[new_index][j]] == false){
					double distances = distance(base_data[knn_graph[new_index][j]],query_data[i]);
					unsigned ii = knn_graph[new_index][j];
					isExist[ii] = true;
					if(pool.size()<L-1){
						Candidate_List new_point = {ii,distances,false};
						pool.push_back(new_point);
					}		
					else if(pool.size() == L-1){
						Candidate_List new_point = {ii,distances,false};
						pool.push_back(new_point);
						...
						if(distances >= pool[pool.size()-1].dis){ //若距离相等，选择序号小的或序号小的排前面 
							...
							for(z = pool.size()-2;z>=0;z--){ 
								if(pool[z].dis <= distances){
									Candidate_List new_point = {ii,distances,false};
									pool.insert(pool.begin()+z+1,new_point);
									break;
								}
							}
							if(z < 0){
								Candidate_List new_point = {ii,distances,false};
								pool.insert(pool.begin(),new_point);
							}
						} 
					}				
				}	
