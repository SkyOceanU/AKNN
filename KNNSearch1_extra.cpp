		...
		while(index < L){
			...
			for(size_t j = 0;j<k;j++){
				if(isExist[knn_graph[new_index][j]] == false){
					double d = distance(base_data[knn_graph[new_index][j]],query_data[i]);
					if(max_dis == 0 || d<=max_dis){
						Candidate_List new_point = {knn_graph[new_index][j],d,false};
